...
const saveListClip = async (user, list, clipId, isLike) => {
  const collectionRef = collection(db, "ListClip");
  let qUser = query(collectionRef);
  qUser = query(qUser, where("user", "==", user), where("listId", "==", list), where("clipId", "==", clipId));
  ...
  try {
    ...
    console.log('Error al verificar/crear el clip en la lista:', error);
    return false;
  }
};

export default saveListClip;
